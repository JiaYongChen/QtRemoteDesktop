...
// 具体压缩器实现类
class Compression::ZlibCompressor : public Compression::ICompressor {
public:
...
class Compression::LZ4Compressor : public Compression::ICompressor {
public:
    QByteArray compress(const QByteArray& input, int level = -1) override {
#ifdef HAVE_LZ4
        if (input.isEmpty()) return {};
...
class Compression::ZstdCompressor : public Compression::ICompressor {
public:
    QByteArray compress(const QByteArray& input, int level = -1) override {
#ifdef HAVE_ZSTD
        if (input.isEmpty()) return {};
...
// 获取压缩器实例的方法
Compression::ICompressor* Compression::getCompressor(Algorithm algorithm) {
    ...
    default:
        return &zlibCompressor; // 默认使用zlib
    }
}
...
QByteArray Compression::compress(const QByteArray &data, Algorithm algorithm, Level level)
{
    ...
    if (!compressor) {
        s_lastError = "Unsupported compression algorithm";
        return QByteArray();
    }
    ...
    if (compressionLevel == NoCompression) {
        ...
        compressionLevel = -1; // 使用默认级别
    }
    
    QByteArray result = compressor->compress(data, compressionLevel);
    if (result.isEmpty() && !data.isEmpty()) {
        s_lastError = "Compression failed";
    }
...
QByteArray Compression::decompress(const QByteArray &compressedData, Algorithm algorithm)
{
    ...
    QByteArray result = compressor->decompress(compressedData);
    if (result.isEmpty() && !compressedData.isEmpty()) {
        s_lastError = "Decompression failed";
    }
