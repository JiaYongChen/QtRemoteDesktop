...
void TestCommunication::initTestCase()
{
    ...
    // 创建工作线程
    m_captureWorker = new ScreenCaptureWorker();
    m_processorWorker = new DataProcessorWorker();
...
void TestCommunication::testFrameDataTransmission()
{
    ...
    QObject::connect(
        ...
        [this](const QImage& frame, qint64 timestamp) {
            ...
            QMetaObject::invokeMethod(m_processorWorker, "processFrame",
                                    Qt::QueuedConnection,
                                    Q_ARG(QSharedPointer<QImage>, sharedFrame),
    ...
    // 创建两个不同的测试图像用于差分压缩测试
    QImage firstImage(800, 600, QImage::Format_RGB32);
    firstImage.fill(Qt::blue);
    
    QImage secondImage(800, 600, QImage::Format_RGB32);
    secondImage.fill(Qt::blue);
    // 在第二个图像上添加一些变化
    QPainter painter(&secondImage);
    ...
    // 发送第一帧数据（建立基准帧）
    QSharedPointer<QImage> firstFramePtr = QSharedPointer<QImage>::create(firstImage);
    qint64 firstTimestamp = QDateTime::currentMSecsSinceEpoch();
    
    QMetaObject::invokeMethod(m_processorWorker, "processFrame",
                             Qt::DirectConnection,
                             Q_ARG(QSharedPointer<QImage>, firstFramePtr),
                             Q_ARG(qint64, firstTimestamp));
    
    // 强制处理事件循环，确保QueuedConnection被执行
    ...
    for (int i = 0; i < 50 && !firstSignalReceived; ++i) {
        QTest::qWait(100);
        firstSignalReceived = (dataReadySpy.count() > 0 || dataReadyZeroCopySpy.count() > 0);
    }
    
    qDebug() << "第一帧信号接收状态:" << firstSignalReceived;
    qDebug() << "dataReady信号数量:" << dataReadySpy.count();
    qDebug() << "dataReadyZeroCopy信号数量:" << dataReadyZeroCopySpy.count();
    
    QVERIFY(firstSignalReceived);
    QVERIFY(dataReadySpy.count() + dataReadyZeroCopySpy.count() >= 1);
    
    // 清空信号监听器
    dataReadySpy.clear();
    dataReadyZeroCopySpy.clear();
    errorSpy.clear();
    
    // 发送第二帧数据（测试差分压缩）
    QSharedPointer<QImage> secondFramePtr = QSharedPointer<QImage>::create(secondImage);
    qint64 secondTimestamp = QDateTime::currentMSecsSinceEpoch();
    
    QMetaObject::invokeMethod(m_processorWorker, "processFrame",
                             Qt::DirectConnection,
                             Q_ARG(QSharedPointer<QImage>, secondFramePtr),
                             Q_ARG(qint64, secondTimestamp));
    ...
    for (int i = 0; i < 50 && !secondSignalReceived; ++i) {
        QTest::qWait(100);
        secondSignalReceived = (dataReadySpy.count() > 0 || dataReadyZeroCopySpy.count() > 0);
    }
    
    // 如果没有收到信号，检查是否有错误信号
    if (!secondSignalReceived) {
        qDebug() << "第二帧信号未收到，检查错误信号...";
        qDebug() << "dataReady信号数量:" << dataReadySpy.count();
        qDebug() << "dataReadyZeroCopy信号数量:" << dataReadyZeroCopySpy.count();
    ...
    if (dataReadySpy.count() > 0) {
        ...
        QCOMPARE(receivedTimestamp, secondTimestamp);
        
        qDebug() << "帧数据传输测试成功（dataReady），第二帧压缩后大小:" << data->size() << "字节";
        ...
        qint64 receivedTimestamp = arguments.at(1).toLongLong();
        bool isDifferential = arguments.at(2).toBool();
        
        QCOMPARE(receivedTimestamp, secondTimestamp);
        
        qDebug() << "帧数据传输测试成功（dataReadyZeroCopy），差分压缩:" << isDifferential;
    }
...
void TestCommunication::testPerformanceStats()
{
    ...
    qDebug() << "  压缩比:" << stats.compressionRatio;
