...
private slots:
    ...
    void test_compressionConsistency();
...
void TestDataConsistency::test_compressionConsistency()
{
    qDebug() << "测试压缩数据一致性";
    ...
    for (int i = 0; i < m_testImages.size(); ++i) {
        ...
        // 测试不同压缩质量下的数据一致性
        QList<int> qualityLevels = {100, 90, 75, 50, 25};
        ...
        for (int quality : qualityLevels) {
            ...
            buffer.open(QIODevice::WriteOnly);
            QVERIFY(originalImage.save(&buffer, "JPEG", quality));
            
            // 解压缩图像
            QImage decompressedImage;
            QVERIFY(decompressedImage.loadFromData(compressedData, "JPEG"));
            
            // 验证基本属性一致性
            QCOMPARE(decompressedImage.width(), originalImage.width());
            QCOMPARE(decompressedImage.height(), originalImage.height());
            
            // 再次压缩并验证数据稳定性
            QByteArray recompressedData;
            QBuffer recompressedBuffer(&recompressedData);
            recompressedBuffer.open(QIODevice::WriteOnly);
            QVERIFY(decompressedImage.save(&recompressedBuffer, "JPEG", quality));
            
            // 验证重复压缩的数据大小相近（JPEG压缩可能有较大差异）
            double sizeDifference = qAbs(compressedData.size() - recompressedData.size()) / 
                                   static_cast<double>(compressedData.size());
            QVERIFY2(sizeDifference < 0.15, // 允许15%的差异（JPEG压缩特性）
                    qPrintable(QString("压缩数据大小差异过大: %1%").arg(sizeDifference * 100)));
        }
        
        qDebug() << QString("图像 %1 压缩一致性验证通过").arg(i);
    }
}
